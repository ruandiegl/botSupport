...
export class FlowExecutionRepository {
  ...
  handoff(conversationId: string, departmentId: string, context: Record<string, unknown>) { return prisma.conversation.update({ where: { id: conversationId }, data: { status: "QUEUED", departmentId, currentFlowNodeId: null, currentStep: "QUEUED", flowContext: context as Prisma.InputJsonValue } }); }
