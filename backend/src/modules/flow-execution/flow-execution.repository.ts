...
export class FlowExecutionRepository {
  ...
  handoff(conversationId: string, departmentId: string, context: Record<string, unknown>) { const now = new Date(); return prisma.conversation.update({ where: { id: conversationId }, data: { status: "QUEUED", departmentId, queuedAt: now, lastActivityAt: now, currentFlowNodeId: null, currentStep: "QUEUED", flowContext: context as Prisma.InputJsonValue } }); }
