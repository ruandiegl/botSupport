...
O adaptador escolhe `send-button-list` ou `send-option-list` por `ZAPI_INTERACTIVE_MODE=auto|button|option`. Em `auto`, listas maiores que três opções usam `send-option-list`; erros retornam para texto numerado. Listas interativas são restritas à conversa privada e nunca são enviadas ao JID do grupo. Menus com categorias tentam o transporte nativo agrupado (`ZAPI_GROUPED_MENU_TRANSPORT=sections`) e, caso a instância rejeite a extensão, repetem a chamada no formato plano documentado antes do fallback textual; use `flat` para desativar a tentativa.

### Categorias, itens e resumo de contato conhecido

Decisões secundárias podem usar `config.decisionMode=CATEGORIES` e armazenar `decisionGroups[]`. O executor preserva `categoryLabel` e `categoryKey` nos itens. O transporte tenta renderizar uma lista em acordeão por categorias; quando o provedor não aceita `optionList.sections`, repete no contrato plano documentado e conserva os nomes das categorias na descrição/fallback. Revisões antigas com `decisionOptions` permanecem inalteradas.
