...
O adaptador escolhe `send-button-list` ou `send-option-list` por `ZAPI_INTERACTIVE_MODE=auto|button|option`. Em `auto`, listas maiores que três opções usam `send-option-list`; erros ou respostas sem `zaapId`/`messageId` retornam para texto numerado. Listas interativas são restritas à conversa privada e nunca são enviadas ao JID do grupo. Menus com categorias usam sempre o contrato plano documentado (`optionList.options`) e preservam o nome da categoria na descrição de cada item; isso evita que a Z-API aceite um campo experimental e renderize somente o prompt.

### Categorias, itens e resumo de contato conhecido

Decisões secundárias podem usar `config.decisionMode=CATEGORIES` e armazenar `decisionGroups[]`. O executor preserva `categoryLabel` e `categoryKey` nos itens. Como a documentação pública da Z-API não define `optionList.sections`, o transporte entrega a lista plana compatível e conserva os nomes das categorias na descrição/fallback. Revisões antigas com `decisionOptions` permanecem inalteradas.
