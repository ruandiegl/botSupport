...
export function useAvailableShortcuts(conversationId: string, q: string, type: ShortcutType | "ALL", enabled: boolean) {
...
export function useRegisterShortcutUse() {
  return useMutation({
    mutationFn: ({ id, conversationId }: { id: string; conversationId: string }) =>
      apiFetch(`/shortcuts/${id}/use`, { method: "POST", body: JSON.stringify({ conversationId }) }),
  });
}
