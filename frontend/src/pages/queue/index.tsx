...
const statusLabels: Record<string, string> = {
  ALL: "Todos os status", QUEUED: "Na fila", IN_PROGRESS: "Em atendimento", BOT: "No bot", CLOSED: "Encerradas",
};

const defaultDateRange: DateRangeValue = { preset: "ALL", dateField: "lastActivityAt", from: "", to: "" };
type MetricFilter = "OPEN" | "QUEUED" | "MINE" | "UNREAD";
...
export default function QueuePage(props?: { onlyMine?: boolean } & Record<string, unknown>) {
  ...
  const [status, setStatus] = useState(onlyMine ? "IN_PROGRESS" : "QUEUED");
  const [metricFilter, setMetricFilter] = useState<MetricFilter | null>(onlyMine ? "MINE" : "QUEUED");
  const [department, setDepartment] = useState("ALL");
  const [search, setSearch] = useState("");
  const [dateRange, setDateRange] = useState<DateRangeValue>(defaultDateRange);
  const [currentPage, setCurrentPage] = useState(1);

  const queryStatus = metricFilter === "QUEUED"
    ? "QUEUED"
    : metricFilter === "MINE"
    ? "IN_PROGRESS"
    : metricFilter === "OPEN" || metricFilter === "UNREAD"
    ? "ALL"
    : status;
  ...
  const activateMetric = (next: MetricFilter) => {
    ...
    if (!isActive && (next === "MINE" || next === "UNREAD" || next === "OPEN")) setStatus(next === "MINE" ? "IN_PROGRESS" : "ALL");
    if (!isActive && next === "QUEUED") setStatus("QUEUED");
  };
  const metricCounts = result?.counts ?? { open: 0, queued: 0, inProgress: 0, bot: 0, closed: 0, mine: 0, unread: 0 };
  ...
  return (
    <div className="content queue-page">
      ...
      <div className="stats">
        <MetricCard label="Em aberto" value={metricCounts.open} note="conversas em aberto" onClick={() => activateMetric("OPEN")} selected={metricFilter === "OPEN"} ariaLabel="Filtrar conversas em aberto" testId="metric-open" />
        <MetricCard label="Na fila" value={metricCounts.queued} note="aguardando atendimento" tone="warning" onClick={() => activateMetric("QUEUED")} selected={metricFilter === "QUEUED"} ariaLabel="Filtrar conversas na fila" testId="metric-queued" />
        <MetricCard label="Com você" value={metricCounts.mine} note="atendimentos sob sua gestão" tone="success" onClick={() => activateMetric("MINE")} selected={metricFilter === "MINE"} ariaLabel="Filtrar meus atendimentos" testId="metric-mine" />
      ...
      <div className="split-layout">
        <div className="panel conversation-list">
          <div className="panel-header"><div className="panel-title"><MessageCircle size={17} /><h2>{onlyMine ? "Conversas assumidas" : queryStatus === "QUEUED" ? "Conversas na fila" : "Conversas recentes"}</h2></div><span className="subtle">{total} registros</span></div>
